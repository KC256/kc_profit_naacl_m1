# model_9_5: model_9からMultiheadAttentionを削除し、特徴量をフラット化して結合
import torch
...
class Actor(nn.Module):
    ...
    def _process_data(self, state):
        ...
        if INPUT_TEXT=="tweetonly":
            ...
            price_history = state[:, PRICE_HISTORY_IDX:EMB2_IDX].view(
        ...
        if INPUT_TEXT=="withSEC" or INPUT_TEXT=="SMLT-FRS-G":
            sec_feats =sec_feats.permute(1, 0, 2, 3) #num_stock, 1, secの数, embeddingsize
        if INPUT_TEXT=="withtimefeatsSEC" or INPUT_TEXT=="SMLT-FRS-G":
        ...
        for i in range(self.num_stocks): #各企業
            ...
            elif INPUT_TEXT=="withtimefeatsSEC" or INPUT_TEXT=="SMLT-FRS-G":
    ...
    def _process_data_2(self, state):
        ...
        sentence_feat = state[:, EMB2_IDX:].view(
            -1, self.num_stocks, N_DAYS, MAX_TWEETS, TWEETS_EMB
    ...
    def forward(self, state):
        # 
        x1 = self._process_data(state) #これを粒度ごとに処理する　process_dataが従来のforwardに相当
        x2 = self._process_data_2(state) 
