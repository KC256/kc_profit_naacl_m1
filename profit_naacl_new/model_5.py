#with2SECtfでSEC2つにアテンションを通したのち，新しい側のtfのみとTLSTM
import numpy as np
import torch
...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        self.linear_c = nn.Linear(2592, num_stocks) #64 * num_stocks + 32
        self.tanh = nn.Tanh()
    ...
    def forward(self, state):
        ...
        for i in range(self.num_stocks): #各企業
            ...
            if INPUT_TEXT=="tweetonly":
                ...
                #267-273を模倣　かつtext_out[i]とsecのtextoutにattn
                temp_sec = sec_feats[i, :, :, :]
                print("temp_sec", temp_sec.shape) #temp_sec torch.Size([1, 2, 384])
                ...
                sec_h2_out = sec_h2_out.permute(1, 0, 2)
                print("sec_h2_out:", sec_h2_out.shape) #sec_h2_out: torch.Size([1, 1, 128])
                sec_attn_out = self.sec_attn2s[i](sec_lstm2_out, sec_h2_out) 
                print("sec_attn_out:", sec_attn_out.shape) #sec_attn_out: torch.Size([1, 128])
                
                
                
                sec_lstmout, (_, _) = self.tsec_lstm1s[i]( #TLSTM
                    sec_attn_out, temp_sec_time_feats, (h_init, c_init) #入力サイズ変更してる
                )
        ...
        full = torch.cat([x_stock, text_out, text_out_2], dim=1)
...
class Critic(nn.Module):
    ...
    def __init__(
        ...
        self.linear_c = nn.Linear(2592, 32)
