# model_6: Fuses price_history directly without LSTM and Linear layers.
import numpy as np
import torch
...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        for i, day_lstm in enumerate(self.sec_lstm2s):
        ...
        final_input_dims = (64 * num_stocks) + (64 * num_stocks) + 32 + (PRICE_HISTORY_DAYS * PRICE_FEATURES * num_stocks)
        self.linear_c = nn.Linear(final_input_dims, num_stocks)
        self.tanh = nn.Tanh()
    ...
    def forward(self, state):
        ...
        for i in range(self.num_stocks):
            ...
            if INPUT_TEXT=="tweetonly":
                ...
                sec_lstm2_out, (sec_h2_out, _) = self.sec_lstm2s[i](temp_sec.to(self.device))
                ...
                sec_lstmout, (_, _) = self.tsec_lstm1s[i](
                    temp_sec, temp_sec_time_feats, (h_init, c_init)
                )
                sec_lstm2_out, (sec_h2_out, _) = self.tsec_lstm2s[i](sec_lstmout)
                sec_h2_out = sec_h2_out.permute(1, 0, 2)
                x2 = self.sec_attn2s[i](sec_lstm2_out, sec_h2_out)
                x2 = self.drop(self.relu(self.sec_linearx1[i](x2)))
                x2 = self.sec_linearx2[i](x2)
        ...
        price_history_flat = price_history.view(self.bs, -1)
        # print("price_history_flat.shape:", price_history_flat.shape)

        x_stock = self.relu(self.linear1(stock_feats))
        x_stock = self.linear2(x_stock)

        full = torch.cat([x_stock, text_out, text_out_2, price_history_flat], dim=1)
...
class Critic(nn.Module):
    ...
    def __init__(
        ...
        final_input_dims = (64 * num_stocks) + (64 * num_stocks) + 32 + (PRICE_HISTORY_DAYS * PRICE_FEATURES * num_stocks)
        self.linear_c = nn.Linear(final_input_dims, 32)
        # * Critic Layers
