# model_7 short-term text, mid-to-long term textを一切持ちいす，stock_priceのみを入力とする
import numpy as np
import torch
...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        self.linear_c_stockonly = nn.Linear(140, num_stocks)
        self.tanh = nn.Tanh()
    ...
    def forward(self, state):
        ...
        print("price_history_flat", price_history_flat.shape)
        full = torch.cat([price_history_flat], dim=1)
        print("full", full.shape)
        full = self.tanh(self.linear_c_stockonly(full))
        return full
