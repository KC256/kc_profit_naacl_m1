# model_8: Uses multi-head attention to fuse features.
import numpy as np
import torch
...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        self.multihead_attn = nn.MultiheadAttention(embed_dim=d_model, num_heads=4, batch_first=True, device=device)
    ...
    def forward(self, state):
        ...
        for i in range(self.num_stocks): #各企業
            ...
            for j in range(num_days): #各日付
                ...
                # print("temp_sent:", temp_sent.shape)
                # print(temp_timefeats)
            ...
            if INPUT_TEXT=="tweetonly":
                ...
                #     # print(temp_sec_time_feats)
                #     temp_sec_time_feats = temp_sec_time_feats.view(1, -1) #ここで形を変換して下行の入力サイズにする
                #     # print(temp_sec_time_feats)
        ...
        print("x_stock.shape", x_stock.shape) # (bs, 64)
        print("text_out.shape", text_out.shape) # (bs, 64 * num_stocks)
        print("text_out_2.shape", text_out_2.shape) # (bs, 64 * num_stocks)
        print("price_out.shape", price_out.shape) # (bs, 16 * num_stocks)

        # Project to d_model
        x_stock_proj = self.proj_stock(x_stock)
        text_out_proj = self.proj_text(text_out)
        text_out_2_proj = self.proj_text2(text_out_2)
        price_out_proj = self.proj_price(price_out)
        print("x_stock_proj.shape", x_stock_proj.shape) # (bs, d_model)
        print("text_out_proj.shape", text_out_proj.shape) # (bs, d_model)
        print("text_out_2_proj.shape", text_out_2_proj.shape) # (bs, d_model)
        print("price_out_proj.shape", price_out_proj.shape) # (bs, d_model)
        # Create sequence for attention
        # (bs, 4, d_model)
        attn_input = torch.stack([x_stock_proj, text_out_proj, text_out_2_proj, price_out_proj], dim=1) #口座状態，ツイート特徴量、決算報告書特徴量、株価特徴量を結合
        ...
        print("attn_output.shape", attn_output.shape) # (bs, 4, d_model)

        # Flatten the output
        full = attn_output.view(self.bs, -1) # (bs, 4 * d_model)
...
class Critic(nn.Module):
    ...
    def forward(self, state, actions):
        ...
        # Create sequence for attention
        # (bs, 4, d_model)
        attn_input = torch.stack([x_stock_proj, text_out_proj, text_out_2_proj, price_out_proj], dim=1)
